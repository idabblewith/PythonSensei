...
def day_078():
    ...
    raw_file = os.path.join(os.path.dirname(__file__), "files", "nobel_prize_data.csv")
    ...
    # Alternative:
    multiple_winners = df_data.groupby(by="full_name").filter(
        lambda x: x["year"].count() >= 2
    )
    ...
    nls(df_data.nlargest(n=1, columns="winning_age"))
    nls(df_data.nsmallest(n=1, columns="winning_age"))
    ...
    box = px.box(
        df_data, x="category", y="winning_age", title="How old are the Winners?"
    )
