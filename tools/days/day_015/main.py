...
def ask_drink():
    choice = nli('What would you like? "espresso", "latte", or "cappuccino"?\nType \'report\' to see resources.').lower()
    while choice not in COFFEE_CHOICES: choice = ask_drink()
...
def check_payment(total_paid, choice):
	...
	if total_paid < COFFEE_CHOICES[choice]:
		nls(f"You are ${round(COFFEE_CHOICES[choice] - total_paid, 2)} short.\nYou've been refunded. Please try again.")
		run_again = True
