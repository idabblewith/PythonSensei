...
def day_040():
    ...
    for destination in sheet_data:
        ...
        flight = flight_search.check_flights(
            ORIGIN_CITY, destination["iataCode"], from_time=tomorrow, to_time=six_later
        )

        if flight == None:
            nls(
                f'{bcolors.WARNING}No direct flights found for {destination["iataCode"]}.{bcolors.ENDC}'
            )
            ...
            if stopover_flight == None:
                ...
                nls(
                    f'{bcolors.WARNING}No stopover flights found for {destination["iataCode"]}.{bcolors.ENDC}'
                )
            elif stopover_flight.price < destination["lowestPrice"]:
                print(f'{bcolors.FAIL}"STOPOVER FLIGHT FOUND"{bcolors.ENDC}')
                nls(
                    f'{bcolors.OKGREEN}{stopover_flight.origin_city}-{destination["city"]} @ ${stopover_flight.price}\n{stopover_flight.out_date} via {stopover_flight.destination_city}{bcolors.ENDC}'
                )
                # stop_to_dest =

                cheap.append(
                    dict(
                        fromCity=stopover_flight.origin_city,
                        toCity=stopover_flight.destination_city,
                        price=stopover_flight.price,
                        fromDate=stopover_flight.out_date,
                        toDate=stopover_flight.return_date,
                        stops=stopover_flight.stop_overs,
                        viaCity=stopover_flight.via_city,
                    )
                )
        elif flight.price < destination["lowestPrice"]:
            nls(
                f"{bcolors.OKGREEN}{flight.origin_city}-{flight.destination_city} @ ${flight.price}\n{flight.out_date}{bcolors.ENDC}"
            )
    ...
    for user in email_list["users"]:
        ...
        print(f'Sending below message to {user["email"]}:\n{message}')
