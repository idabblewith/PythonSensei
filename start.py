...
if __name__ == "__main__":
    ...
    print(f"{bcolors.HEADER}IMPORTANT: Please make sure you are in the correct directory for this file.{bcolors.ENDC}")
    ...
    while cont == True:
        if first == True:
            start_process()
            first = False
            nls(f"{bcolors.OKGREEN} {'*'*20} {bcolors.ENDC} PROGRAM COMPLETE! {bcolors.OKGREEN} {'*'*20} {bcolors.ENDC} ")  
        else:
            try:
                go_again = nli("If you would like to try another program press enter.\nTo exit type any other character and press enter.\n")
                cls()
